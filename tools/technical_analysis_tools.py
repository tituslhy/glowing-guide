...
def technical_analysis(
    ticker: str,
    period: Optional[Literal["1mo","3mo","6mo","1y","5y","10y", "ytd","max"]] = "ytd") -> pd.DataFrame:
    """
    Conducts technical analysis on a particular stock ticker.
    ...
    df = get_AccDistIndex(df=df)
    df = get_aroon_indicator(df=df)
    df = get_bollinger_bands(df=df)
    df = get_ichimoku_indicator(df=df)
    df = get_macd(df=df)
    df = get_stoch_oscillator(df=df)
    df = get_stoch_rsi(df=df)
...
def get_AccDistIndex(df: pd.DataFrame):
    """
